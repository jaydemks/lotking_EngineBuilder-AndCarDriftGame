...
test('Character template pack loads in the browser runtime', async ({page}) => {
  ...
  const result = await page.evaluate(() => {
    ...
    return {
      ...
      hasLegacyCameraHelper:template.graph.logicScene.elements.some(element=>element.id==='camera_anchor'),
      cameraRoles:template.graph.logicScene.elements.filter(element=>element.cameraRigRole).map(element=>element.cameraRigRole).sort(),
      hasSoccerParts:template.graph.logicScene.elements.some(element=>/ball|goal|penalty/i.test((element.id||'')+' '+(element.name||''))),
  ...
  expect(result).toEqual({
    ...
    npcPreset:'civil',
    hasLegacyCameraHelper:false,
    cameraRoles:['character-first','character-third'],
    hasSoccerParts:false,
    houseCount:8,
    hasScooter:true,
